...
export function haversineKm(start: [number, number], end: [number, number]) {
...
export function offsetPoint(point: [number, number], bearingRadians: number, distanceKm: number): [number, number] {
...
export function bearingRadians(start: [number, number], end: [number, number]) {
  const lat1 = (start[1] * Math.PI) / 180;
  const lat2 = (end[1] * Math.PI) / 180;
  const deltaLon = ((end[0] - start[0]) * Math.PI) / 180;
...
export function isWaypointAlongCorridor(waypoint: [number, number], start: [number, number], end: [number, number], maxDetourRatio = 1.4, maxDetourKm = 60) {
  const directDistance = haversineKm(start, end);
  const viaWaypoint = haversineKm(start, waypoint) + haversineKm(waypoint, end);

  return viaWaypoint <= directDistance * maxDetourRatio + maxDetourKm;
}

export function isReasonableRoute(coordinates: [number, number][], start: [number, number], end: [number, number], maxDetourRatio = 1.45, maxDetourKm = 80) {
