...
const codegenNativeComponentShim = path.resolve(
  root,
  "src/shims/codegenNativeComponent.ts",
);
const assetsRegistryShim = path.resolve(
  root,
  "src/shims/assetsRegistry.ts",
);
const normalizeCssColorShim = path.resolve(
  root,
  "src/shims/normalizeCssColor.ts",
);
...
export default defineConfig({
  plugins: [react(), cjsDefaultExportPlugin(), tailwindcss()],
  define: {
